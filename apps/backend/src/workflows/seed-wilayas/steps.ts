...
const wilayaEnglishNames: Record<number, string> = {
  ...
  56: "Touggourt", 57: "Djanet", 58: "El Bayadh El Djadid", 59: "Oum El Bouaghi El Djadid", 
  60: "Sidi Bel Abbès El Djadid", 61: "Khenchela El Djadid", 62: "Naâma El Djadid", 63: "Aïn Témouchent El Djadid",
  64: "Ghardaïa El Djadid", 65: "Relizane El Djadid", 66: "El M'Ghair El Djadid", 67: "El Meniaa El Djadid",
...
export async function seedWilayasStepHandler(
  ...
  try {
    ...
    for (const wilaya of geoWilayas) {
      ...
      const { data: [existing] } = await dbService.graph({
        entity: "wilaya_rate",
        fields: ["id"],
      ...
      if (!existing) {
        const record = await activeModuleService.create(basePayload);
