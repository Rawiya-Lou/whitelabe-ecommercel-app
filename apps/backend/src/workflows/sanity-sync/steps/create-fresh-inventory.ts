...
export const createFreshInventoryStep = createStep(
  ...
  async (input: CreateFreshInventoryInput, { container }) => {
    ...
    // Check upstream structural input conditions
    if (input.inventoryItemExists && input.preexistingInventoryItemId) {
      logger.info(
        `[Sanity Sync] Reusing pre-existing inventory entry for SKU: [${input.sku}]`,
      );
    ...
    // Query the real database by SKU right before mutating to prevent unique key violations
    const [existingItemBySku] = await inventoryService.listInventoryItems({
      sku: [input.sku],
    });

    if (existingItemBySku) {
      logger.info(
        `[Sanity Sync] SKU [${input.sku}] found in database during pre-write validation. Syncing quantity balances.`,
      );
    ...
    logger.info(
      `[Sanity Sync] Spawning new inventory ledger entry row for SKU: [${input.sku}]`,
    );

    // Uses lightweight direct module insertion instead of heavy nested sub-workflows
  ...
  async (compensateData, { container }) => {
    ...
    logger.warn(
      `[Workflow Rollback] Purging newly created inventory item ID: [${compensateData.inventoryItemId}] due to downstream failure.`,
    );
    await inventoryService.deleteInventoryItems([
      compensateData.inventoryItemId,
    ]);
  },
);
